...
pub fn aggregate_proofs_with_public_inputs<E: Pairing + std::fmt::Debug>(
    ...
    // Compute summed and KZG commitment if public inputs provided
    ...
    let summed = (0..l)
        ...
        .map(|i| {
            ...
            for j in 1..public_inputs.len() {
                ...
                ai.mul_assign(&r_vec[j]);
                c.add_assign(&ai);
            }
            c
        })
        .collect::<Vec<_>>();
    let totsi = <<E as Pairing>::G1 as VariableBaseMSM>::msm(&pvk.vk.gamma_abc_g1[1..], &summed)
        .unwrap()
        .into_affine();
